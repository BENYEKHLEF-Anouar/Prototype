...
$rate_limit_file = '../config/admin_login_attempts.json';
$max_attempts = 5;
$lockout_time = 900; // 15 minutes

/**
 * Checks if the admin has exceeded login attempts.
 * @param string $identifier Unique admin identifier (email_IP).
...
/**
 * Updates admin login attempt data in JSON file.
 * @param string $identifier Unique admin identifier.
...
function update_login_attempt($identifier, $success) {
    ...
    if (!file_put_contents($rate_limit_file, json_encode($attempts, JSON_PRETTY_PRINT))) {
        log_error("Failed to write admin login attempt for $identifier to $rate_limit_file");
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["submit"])) {
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        log_error("CSRF validation failed for admin login attempt: IP {$_SERVER['REMOTE_ADDR']}");
    } else {
        ...
        if (!check_rate_limit($identifier)) {
            ...
            log_error("Rate limit exceeded for admin $identifier");
        } else {
            ...
            if (empty($errors)) {
                try {
                    $stmt = $pdo->prepare("SELECT idAdministrateur, nomAdministrateur, prenomAdministrateur, emailAdministrateur, motDePasse FROM Administrateur WHERE emailAdministrateur = :email");
                    ...
                    if ($admin && password_verify($password, $admin['motDePasse'])) {
                        $_SESSION['admin'] = [
                            ...
                            'nom' => $admin['nomAdministrateur'],
                            'prenom' => $admin['prenomAdministrateur'],
                            'email' => $admin['emailAdministrateur'],
                            'role' => 'admin'
                        ];
                        $success = 'Connexion réussie ! Vous allez être redirigé vers le tableau de bord.';
                        update_login_attempt($identifier, true);
                    } else {
                        $errors['general'] = "Email ou mot de passe incorrect.";
                        update_login_attempt($identifier, false);
                        log_error("Failed admin login attempt: $email from IP {$_SERVER['REMOTE_ADDR']}");
                    }
                } catch (PDOException $e) {
                    log_error("Admin login error for $email: " . $e->getMessage());
...
<head>
    ...
    <title>Mentora - Connexion Administrateur</title>
...
<body>
    ...
    <div class="main-content">
        <div class="container admin-login-container" data-aos="fade-down" data-aos-duration="1600">
            <div class="title_container">
                <h2 class="title admin-title">Connexion Administrateur</h2>
                <span class="subtitle">Accédez au tableau de bord administrateur avec vos identifiants.</span>
            </div>
            ...
            <form action="admin_login.php" method="post">
                ...
                <div class="input-group">
                    ...
                    <div class="relative">
                        <input type="email" name="email" value="<?= sanitize($email) ?>" placeholder="admin@mentora.com" class="<?= get_field_class('email') ?>" />
                        <div>
                ...
                <button class="btn admin-btn" name="submit" type="submit">
            ...
            <p class="switch-form">Pas de compte administrateur ? <a href="admin_register.php">Créer un compte</a>.</p>
            <p class="switch-form" style="margin-top: 1rem;"><a href="login.php">← Retour à la connexion utilisateur</a></p>
        </div>
    </div>
...
<script>
    ...
    const switchToRegisterLink = document.querySelector('.switch-form a[href="admin_register.php"]');

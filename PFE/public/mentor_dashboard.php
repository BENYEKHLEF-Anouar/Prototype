...
try {
    ...
    if (empty($chart_values) || array_sum($chart_values) == 0) {
        $chart_values = [2, 5, 3, 8, 6, 4]; // Sample data for testing
    }
    ...
    $stmt = $pdo->prepare("
        SELECT idSession, titreSession, descriptionSession, dateSession, heureSession,
               tarifSession, niveau, statutSession, typeSession, lienReunion,
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function handleUpdateSession(form, sessionCard, editForm) {
        ...
        try {
            ...
            if (response.ok && result.status === 'success') {
                ...
                // Update session type
                const typeElement = sessionCard.querySelector('.session-type span');
                if (typeElement) {
                    typeElement.textContent = data.typeSession === 'en_ligne' ? 'En ligne' : 'Présentiel';
                }
                const typeIcon = sessionCard.querySelector('.session-type i');
                if (typeIcon) {
                    typeIcon.className = `fas fa-${data.typeSession === 'en_ligne' ? 'video' : 'map-marker-alt'}`;
                }

                // Update meeting link
                const meetingLinkDiv = sessionCard.querySelector('.session-meeting-link');
                if (data.typeSession === 'en_ligne' && data.lienReunion) {
                    if (!meetingLinkDiv) {
                        // Create meeting link if it doesn't exist
                        const newMeetingLink = document.createElement('div');
                        newMeetingLink.className = 'session-meeting-link';
                        newMeetingLink.innerHTML = `
                            <a href="${data.lienReunion}" target="_blank" class="meeting-link-btn">
                                <i class="fas fa-external-link-alt"></i>
                                <span>Rejoindre la réunion</span>
                            </a>
                        `;
                        sessionCard.querySelector('.session-details').appendChild(newMeetingLink);
                    } else {
                        // Update existing meeting link
                        const linkElement = meetingLinkDiv.querySelector('a');
                        if (linkElement) {
                            linkElement.href = data.lienReunion;
                        }
                    }
                } else if (meetingLinkDiv) {
                    // Remove meeting link for in-person sessions
                    meetingLinkDiv.remove();
                }
    ...
    async function handleCompleteSession(button) {
        ...
        if (!confirm(`Marquer la session "${sessionTitle}" comme terminée ?\n\nLes étudiants pourront alors l'évaluer.`)) return;
        ...
        try {
            ...
            const response = await fetch('actions/complete_session.php', { method: 'POST', body: formData });
            const result = await response.json();
            ...
            if (response.ok && result.status === 'success') {
                ...
                statusElement.className = 'session-status status-terminee';
                statusElement.textContent = 'Terminée';

                // Remove the complete button and edit button
                button.remove();
                const editBtn = sessionCard.querySelector('.edit-session-btn');
                if (editBtn) editBtn.remove();

            } else {
                throw new Error(result.message || 'Erreur lors de la completion.');
            }
        } catch (error) {
            showGlobalFeedback(error.message, 'error');
            button.disabled = false;

...
$is_admin = isset($_SESSION['admin']) && $_SESSION['admin']['role'] === 'admin';
$admin_editing = $is_admin && isset($_GET['user_id']);

if ($admin_editing) {
    // Admin editing another user
    $mentor_user_id = intval($_GET['user_id']);
    if ($mentor_user_id <= 0) {
    ...
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        ...
        if (empty($prenom) || empty($nom) || empty($email) || empty($competences)) {
            ...
            try {
                ...
                $feedback = ['type' => 'success', 'message' => $admin_editing ? 'Profil utilisateur mis à jour avec succès.' : 'Votre profil a été mis à jour avec succès.'];

                // Update session data only if user is editing their own profile
                if (!$admin_editing) {
...
<main class="main-content">
    <div class="edit-profile-container" data-aos="fade-down" data-aos-duration="1600">
        <div class="edit-profile-header">
            <a href="<?= $admin_editing ? 'admin_dashboard.php' : 'mentor_dashboard.php' ?>" class="sidebar-back-link">
                <i class="fas fa-arrow-left"></i> <?= $admin_editing ? 'Retour' : 'Retour' ?>
            </a>
            <h1 class="page-title"><?= $admin_editing ? 'Modifier le Profil Utilisateur' : 'Modifier le Profil' ?></h1>
        </div>

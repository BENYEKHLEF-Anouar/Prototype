...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST["submit"])) {
    ...
    if (!validate_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        log_error("CSRF validation failed for admin registration attempt: IP {$_SERVER['REMOTE_ADDR']}");
    } else {
        ...
        if (empty($errors)) {
            try {
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM Administrateur WHERE emailAdministrateur = :email");
                $stmt->bindParam(':email', $email, PDO::PARAM_STR);
                $stmt->execute();
                if ($stmt->fetchColumn() > 0) {
                    $errors['general'] = "Cette adresse email est déjà utilisée par un administrateur.";
                } else {
                    $nameParts = explode(' ', $fullname, 2);
                    $prenomAdministrateur = $nameParts[0];
                    $nomAdministrateur = $nameParts[1] ?? '';
                    $hashed_password = password_hash($password, PASSWORD_DEFAULT);

                    $stmtAdmin = $pdo->prepare("INSERT INTO Administrateur (nomAdministrateur, prenomAdministrateur, emailAdministrateur, motDePasse) VALUES (:nom, :prenom, :email, :password)");
                    $stmtAdmin->execute([
                        ':nom' => $nomAdministrateur,
                        ':prenom' => $prenomAdministrateur,
                        ':email' => $email,
                        ':password' => $hashed_password
                    ]);

                    $success = 'Compte administrateur créé ! Vous allez être redirigé vers la connexion.';
                    log_error("Admin registered: $email. Auto-verified for development.");
                }
            } catch (PDOException $e) {
                log_error("Admin registration error for $email: " . $e->getMessage());
...
<head>
    ...
    <title>Mentora - Inscription Administrateur</title>
...
<body>
    ...
    <div class="admin-header">
        <i class="fas fa-shield-alt"></i> ESPACE ADMINISTRATEUR - INSCRIPTION
    </div>
    ...
    <div class="main-content">
        <div class="container admin-register-container" data-aos="fade-down" data-aos-duration="1600">
            <div class="title_container">
                <h2 class="title admin-title">Inscription Administrateur</h2>
                <span class="subtitle">Créez votre compte administrateur pour gérer la plateforme.</span>
            </div>
            ...
            <form action="admin_register.php" method="post">
                ...
                <div class="form-row">
                    ...
                    <div class="input-group">
                        ...
                        <div class="relative">
                            <input type="email" name="email" placeholder="admin@mentora.com" value="<?= sanitize($email) ?>" class="<?= get_field_class('email') ?>" />
                ...
                <button class="btn admin-btn" type="submit" name="submit">
                    ...
                    <span class="text">Créer le compte administrateur</span>
                </button>
            </form>

            <div class="terms">En créant un compte administrateur, vous acceptez nos <a href="#">Conditions d'utilisation</a> et notre <a href="#">Politique de confidentialité</a>.</div>
            <div class="separator"><hr class="line" /><span>Ou</span><hr class="line" /></div>
            <p class="switch-form">Déjà un compte administrateur ? <a href="admin_login.php">Connectez-vous ici</a>.</p>
            <p class="switch-form" style="margin-top: 1rem;"><a href="register.php">← Retour à l'inscription utilisateur</a></p>
        </div>
    </div>
    ...
    <script>
        ...
        const switchToLoginLink = document.querySelector('.switch-form a[href="admin_login.php"]');

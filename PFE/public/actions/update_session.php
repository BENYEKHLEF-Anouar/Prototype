...
$typeSession = $_POST['typeSession'] ?? 'en_ligne';
$lienReunion = trim($_POST['lienReunion'] ?? '');

if (!$sessionId) {
    send_json_error('ID de session invalide.');
}

if (empty($titreSession) || empty($descriptionSession) || empty($dateSession) || empty($heureSession) || $tarifSession === false || $tarifSession < 0 || empty($niveau)) {
    send_json_error('Tous les champs sont obligatoires et le tarif doit être positif.');
}

// Validate session type
if (!in_array($typeSession, ['en_ligne', 'presentiel'])) {
    send_json_error('Type de session invalide.');
}

// Validate meeting link for online sessions
if ($typeSession === 'en_ligne' && empty($lienReunion)) {
    send_json_error('Le lien de réunion est obligatoire pour les sessions en ligne.');
}

if ($typeSession === 'en_ligne' && !filter_var($lienReunion, FILTER_VALIDATE_URL)) {
    send_json_error('Le lien de réunion doit être une URL valide.');
}
...
try {
    ...
    $stmt = $pdo->prepare("
        UPDATE Session
        SET titreSession = ?, descriptionSession = ?, dateSession = ?, heureSession = ?, tarifSession = ?, niveau = ?, typeSession = ?, lienReunion = ?
        WHERE idSession = ? AND idMentorAnimateur = ?
    ");
    $success = $stmt->execute([$titreSession, $descriptionSession, $dateSession, $heureSession, $tarifSession, $niveau, $typeSession, $lienReunion, $sessionId, $mentorId]);
    ...
    if ($success && $stmt->rowCount() > 0) {
        send_json_success('Session modifiée avec succès.', [
            ...
            'lienReunion' => htmlspecialchars($lienReunion)
        ]);
    } else {
